...
def login():
    ...
    # For now, let's assume it always redirects to the index page
    return redirect(url_for('index'))

# Route for the index page which will be shown after a successful login
@app.route('/index')
def index():
...
def calculate_green_light_duration(vehicle_counts_list):
    ...
    max_cycle_time = 180   # maximum total cycle time in seconds
    ...
    if total_initial_time > max_cycle_time:
        ...
        green_light_durations = [round(apply_exponential_scaling(time, i, len(initial_green_times)) * scaling_factor, 2) for i, time in enumerate(initial_green_times)]
    else:
        green_light_durations = [round(apply_exponential_scaling(time, i, len(initial_green_times)), 2) for i, time in enumerate(initial_green_times)]
